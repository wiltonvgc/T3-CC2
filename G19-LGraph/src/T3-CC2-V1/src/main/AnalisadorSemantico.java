...
public class AnalisadorSemantico extends LGraphBaseVisitor<String> {
	...
	ArrayList<String> grafos_nos_simples,grafos_nos_atributos;
	...
	public AnalisadorSemantico(TabelaDeSimbolos t,SaidaParser sp,PilhaDeTabelas p){
		...
		this.grafos_nos_simples = new ArrayList<String>();
		this.grafos_nos_atributos = new ArrayList<String>();
	}
	...
	if(filhos!=null){
		...
		/* FOREACH */
			...
			/* verifica se grafo ja foi criado */

			if(!this.grafos_criados.contains(id_grafo) && this.pilhaTabs.getTipo(id_grafo).equals("graph")){
				sp.println("Erro: grafo " + id_grafo + " não inicializado com create", "semantico");
			}
		...
		if(comando==5){
			...
			}else if(t!=null && t=="string"){
				if(!tipo_var_atr.equals(t)){
					sp.println("Erro: incompatibilidade de tipo em atribuicao de " + var_atribuicao, "semantico");
				}
				
			}
			else if(t!=null && t.equals("nodes_com_atributos")){
				if(!tipo_var_atr.equals(t)){
					sp.println("Erro: incompatibilidade de tipo em atribuicao de " + var_atribuicao, "semantico");
				}
			}
		...
		if(comando==5){
			if(ctx!=null && ctx.atribuicao()!=null && ctx.atribuicao().nodes_atributos_atribuicao()!=null){
	...
	public String visitExp_relacional(Exp_relacionalContext ctx) {
		...
		if(ctx.op1_id!=null){
			...
			if(!this.pilhaTabs.existeSimbolo(ctx.op1_id.getText())){
				sp.println("Erro: " + ctx.op1_id.getText() + " não declarado", "semantico");
			}
		...
		if(ctx.op2_id!=null){
			...
			if(!this.pilhaTabs.existeSimbolo(ctx.op2_id.getText())){
				sp.println("Erro: " + ctx.op2_id.getText() + " não declarado", "semantico");
			}
	...
	public String visitExp_igualdade(Exp_igualdadeContext ctx) {
		...
		if(tipo1!=null && tipo2!=null && (tipo1.equals("int") || tipo1.equals("float")) && tipo2.equals("string")){
				sp.println("Erro: comparação invalida entre números e string", "semantico");
		}else if(tipo1!=null && tipo2!=null && (tipo2.equals("int") || tipo2.equals("float")) && tipo1.equals("string")){
				sp.println("Erro: comparação invalida entre números e string", "semantico");
		}else if(tipo1!=null && tipo2!=null && (tipo1.equals("graph" )|| tipo1.equals("edges") || tipo1.equals("nodes") ||  tipo1.equals("nodes_com_atributos"))){
	...
	public String visitParametros_create(Parametros_createContext ctx) {
		...
		if(ctx.v2.IDENT()!=null){
			String id = ctx.v2.IDENT().getText();
			/* Se for nos sem atributos */
			if(this.pilhaTabs.getTipo(id).equals("nodes")){
				this.grafos_nos_simples.add(this.grafo_create);
				
			}
			/* Senao */
			else if(this.pilhaTabs.getTipo(id).equals("nodes_com_atributos")){
				this.grafos_nos_atributos.add(this.grafo_create);
			}
		}
	...
	public String visitParametros_update(Parametros_updateContext ctx) {
		...
		/* Verifica compatibilidade entre parametros */
		/* NODES */
		if(tipo1!=null && tipo1.equals("nodes") && this.grafos_nos_atributos.contains(this.grafo_update)){
			sp.println("Erro: " + this.grafo_update + " só pode conter vértices com atributos", "semantico");
		}else if(tipo1!=null && tipo1.equals("nodes_com_atributos") && this.grafos_nos_simples.contains(this.grafo_update)){
			sp.println("Erro: " + this.grafo_update + " só pode conter vértices sem atributos", "semantico");
		}
		
		else if(tipo1!=null && !tipo1.equals("nodes") && !tipo1.equals("nodes_com_atributos")){
			sp.println("Erro: incompatibilidade de tipo em parametro nodes de update","semantico");		
		}
		/* EDGES */
		if(tipo2!=null && !tipo2.equals("edges")){
		...
		/* Cria estrtura de dados de grafo com atributos e seus tipos caso seja um grafo com nodes com atributos */
		if(tipo1!=null && tipo1.equals("nodes_com_atributos")){
			Nodes no=null;
		...
		if(ctx!=null){
			if(ctx.IDENT()!=null){
				...
				if(!this.pilhaTabs.existeSimbolo(tipo)){
					...
					tipo = this.pilhaTabs.getTipo(tipo);
				}
			}
